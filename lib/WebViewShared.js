...
var defaultOriginWhitelist = ["http://*", "https://*", "intent://*"];
...
var compileWhitelist = function (originWhitelist) {
  return __spreadArrays(["about:blank"], originWhitelist || []).map(
    originWhitelistToRegex
  );
};
var createOnShouldStartLoadWithRequest = function (
  loadRequest,
  originWhitelist,
    ...
    if (!passesWhitelist(compileWhitelist(originWhitelist), url)) {
      Linking.canOpenURL(url)
        .then(function (supported) {
          ...
          }else if (!supported && url.startsWith("intent://gopay")) {
            let deeplinkGopay = "https://gojek.link/";
            let splittingUrl = url.split("://");
            return Linking.openURL(deeplinkGopay + splittingUrl[1]);
          }
